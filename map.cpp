...
Door Map::get_door(unsigned short x, unsigned short y)
{
	unsigned int i = 0;
...
void Map::add_temp_sprite(unsigned short itex, float x, float y, unsigned short size)
{
	unsigned int index = sprites.size();
	sprites.push_back(Sprite());
	sprites.at(index).x = x;
...
{
	//player position
	int px = unsigned short(player_x);
	int py = unsigned short(player_y);
	if(dist[px+py*w] != 0)
	...
	for(unsigned int i = 0; i < sprites.size(); i++)
	{
		if(sprites.at(i).type == Enemy)
		{
			unsigned short x = unsigned short(sprites.at(i).x);
			unsigned short y = unsigned short(sprites.at(i).y);
