...
void Player::handle_events(float dt)
{
    ...
    {
        if(map->get_tile(short(nx), short(y)) == ' ')
            x = nx;
        if(map->get_tile(short(x), short(ny)) == ' ')
            y = ny;
    }
}
...
void Player::Fire()
{
    ...
    while(!hit_wall && dist < 10) //makes the ray move forward step by step
    {
        ...
        if(map->get_tile(short(ray_x), short(ray_y)) != ' ') //the current tile is not empty, we hit a wall
        {
            hit_wall = true;
            if(map->get_tile(short(ray_x), short(ray_y)) == '2')
            {
                wall_destruct = true;
                map->set_tile(short(ray_x), short(ray_y), ' ');
                map->add_temp_sprite(7, short(ray_x) + 0.5, short(ray_y) + 0.5, 1000);
