...
void Player::handle_events(float dt)
{
    ...
    {
        if(map->get_tile(unsigned short(nx), unsigned short(y)) == ' ')
            x = nx;
        if(map->get_tile(unsigned short(x), unsigned short(ny)) == ' ')
            y = ny;
    }
}
...
void Player::Fire()
{
    ...
    while(!hit_wall && dist < 10) //makes the ray move forward step by step
    {
        ...
        if(map->get_tile(unsigned short(ray_x), unsigned short(ray_y)) != ' ') //the current tile is not empty, we hit a wall
        {
            hit_wall = true;
            if(map->get_tile(unsigned short(ray_x), unsigned short(ray_y)) == '2')
            {
                wall_destruct = true;
                map->set_tile(unsigned short(ray_x), unsigned short(ray_y), ' ');
                map->add_temp_sprite(7, unsigned short(ray_x) + 0.5, unsigned short(ray_y) + 0.5, 1000);
