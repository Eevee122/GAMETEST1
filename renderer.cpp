...
bool Renderer::init_sdl(const char* title, unsigned shortwidth, unsigned shortheight)
{
    //24 bits colors
    unsigned char bpp = 3;
...
void Renderer::draw(unsigned int fps)
{
    ...
    for(int i = 0; i < screen_w; i++) //for each vertical column of pixel
    {
        ...
        while(!hit_wall && dist < 40) //makes the ray move forward step by step
        {
            ...
            {
                hit_wall = true;
                unsigned shorttile_id = map->get_tile(ushort(ray_x), ushort(ray_y)) - '0';
    ...
    if(menu->current == None)
    {
        ...
        middle += 30;
        unsigned shortcrosshair_size = 8;
...
void Renderer::draw_2d_sprite(unsigned shortitex, unsigned shortx, unsigned shorty, float size)
{
...
void Renderer::draw_text(unsigned shortx, unsigned shorty, std::string text, bool big_text, SDL_Color ttf_color)
{
...
Uint32 Renderer::get_pixel_tex(unsigned shortitex, unsigned shortx, unsigned shorty, bool sprite)
{
...
void Renderer::set_pixel(unsigned shortx, unsigned shorty, Uint32 pixel)
{
